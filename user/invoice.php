...
<head>
    ...
    <style>
    /* Notification Menu Styles with Spacing and Download Button */
    .notification-menu {
            position: absolute;
            top: 100px;
            right: 250px;
...
.notification-menu ul li:last-child {
    border-bottom: none;
    margin-bottom: 0; /* Remove margin for the last notification */
}
...
// Display the appointments
            while ($row = $result->fetch_assoc()) {
                 echo '<tr>';
                 echo '<td>You  Appoint ' . $row['doctor_fullname'] .' at ' . $row['appointment_time'] .' in '.$row['appointment_date']. '</td>';
                echo '<td><a class="download-button" href="download_pdf.php?appointment_id=' . $row['appointment_id'] . '">Download PDF</a></td><br><br>';
                echo '</tr>';
            }
            ?>
        </ul>
